...
public class SelangorPlayers implements Runnable {
	List<String> link;
	static int totalplayer;

	public SelangorPlayers(List<String> line) {
		this.link = line;

	}

	public SelangorPlayers() {
		...
		for (int i = 0; i < link.size(); i++) {
			if (validTable(link.get(i))) {
				...
				try {
					...
					System.out.format(format, a, category, selangorplyer);
					totalplayer += selangorplyer;
					selangorplyer = 0;
				} catch (Exception e) {
	...
	public int getGrandTotal() {
		...
		return SelangorPlayers.totalplayer;
	}
