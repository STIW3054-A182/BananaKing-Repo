...
public class KelantanPlayers implements Runnable {
	List<String> link;
	static int totalplayer;

	public KelantanPlayers(List<String> line) {
		this.link = line;

	}

	public KelantanPlayers() {
		...
		for (int i = 0; i < link.size(); i++) {
			if (validTable(link.get(i))) {
				...
				try {
					...
					System.out.format(format, a, category, kelantanplyer);
					totalplayer += kelantanplyer;
					kelantanplyer = 0;
				} catch (Exception e) {
	...
	public int getGrandTotal() {
		...
		return KelantanPlayers.totalplayer;
	}
