...
public class MelakaPlayers implements Runnable {
	List<String> link;
	static int totalplayer;

	public MelakaPlayers(List<String> line) {
		this.link = line;

	}

	public MelakaPlayers() {
		...
		for (int i = 0; i < link.size(); i++) {
			if (validTable(link.get(i))) {
				...
				try {
					...
					System.out.format(format, a, category, melakaplyer);
					totalplayer += melakaplyer;
					melakaplyer = 0;
				} catch (Exception e) {
	...
	public int getGrandTotal() {
		...
		return MelakaPlayers.totalplayer;
	}
